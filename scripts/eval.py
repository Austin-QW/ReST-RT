...
def main():
    for ckpt in [5000,6000,6750]:
        ...
        model = AutoModelForCausalLM.from_pretrained(model_name_or_path, use_safetensors=True, device_map={"":0}, torch_dtype=torch.float16)
        ...
        for task in ['d2p','p2d','p2d_reverse','d2p_reverse']:
            ...
            # output_name = f"{task}.json" for base model 
